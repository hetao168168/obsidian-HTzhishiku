...
2. 询问用户安装位置,把通过测试的 skill 复制过去 — **没有这一步,产出的 skill 无法被真正调用**。安装目录按运行环境选择:
   - WorkBuddy: 用户级 `~/.workbuddy/skills/` 或项目级 `<workspace>/.workbuddy/skills/`
   - Claude Code: 用户级 `~/.claude/skills/` 或项目级 `.claude/skills/`
   - Cursor: 项目级 `.cursor/skills/`
   - 在 WorkBuddy 中蒸馏时,默认把产物安装到 `~/.workbuddy/skills/<skill-slug>/`
